   ...
   if(validationResult.error){
    ...
    try{
        ...
        let workspace= await new WorkspaceSchema({name:name,description:description,picture:picture,creator:req.user._id});
         ...
         }).catch(err =>{return res.status(500).json({error:"Unexpected error occurred! Try again"})});
...
const addUserWorkspaces = async (req,res) =>{
   ...
   if(workspaces.length !== 5){
      ...
      userWorkspace.save()
      .then(() =>{
         res.status(200).json({message:"Your workspaces has been added successfully"});

      }).catch(err =>{return res.status(400).json({error:"An internal server error occurred! Try again"})});  
   }
...
const getUserWorkspaces= async (req,res) =>{
 try{
   let userWorkspaces=await  UserWorkspaceSchema.find({user:req.user._id}).populate({path:"workspaces",select:"name description picture"});
   return res.status(200).json({userWorkspaces:userWorkspaces});
    
  }catch(err){
      console.log("Getting userWorkspaces error: ",err);
